...
const HeaderUI: React.FC<Props> = ({
  ...
  const handleMobileItemClick = (item: UIItem, e: React.MouseEvent<HTMLAnchorElement>) => {
    e.preventDefault();
    if (item.isRoute) {
      onNavClick(item);
      onToggleMenu(); 
    ...
    onNavClick(item);
    onToggleMenu();
    requestAnimationFrame(() => scrollToId(item.to));
  };
  ...
  const isHomePath = (to: string) => to === '/' || to === '/home' || to === '';
      ...
      <nav ref={navRef as any} aria-label="Primary" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-14 lg:h-16">
          ...
          <div className="hidden lg:flex items-center gap-6 ml-auto pr-[135px]">
            {mainNav.map((item) => {
              const isActiveRoute =
                item.isRoute && typeof window !== 'undefined'
          ...
          <div className="hidden md:flex items-center gap-2">
            <Globe className={['h-4 w-4', textDark ? 'text-gray-600' : 'lg:text-white text-gray-600'].join(' ')} />
        ...
        {isMenuOpen && (
          <>
            ...
            <div id="mobile-menu" className="fixed inset-0 z-[110] overflow-y-auto lg:hidden" aria-modal="true" role="dialog">
              <div className="flex justify-end items-center h-14 px-4 border-b">
                <button onClick={onToggleMenu} className="p-2 rounded-md focus:outline-none" aria-label="Close menu">
              ...
              <div className="pt-2 pb-6 space-y-1">
                {mainNav.map((item) => {
                  ...
                  const isHomeItem = isRoute && isHomePath(item.to);

                  return (
                    <div key={`${item.to}-route`}>
                      <a
                        href={item.to}
                      ...
                      {isHome && isHomeItem && anchorNav.length > 0 && (
                        <div className="mt-1 mb-2">
                          {anchorNav.map((anc) => {
                            const isAncActive = activeAnchor === `#${anc.to}`;
                            return (
                              <a
                                key={`${anc.to}-anchor`}
                                href={`#${anc.to}`}
                                onClick={(e) => handleMobileItemClick(anc, e)}
                                className={`block pl-8 pr-4 py-2 text-base transition ${
                                  isAncActive ? 'text-blue-600 font-semibold' : 'text-gray-800 hover:text-blue-600'
                                }`}
                              >
